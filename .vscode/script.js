...
// Funções para adicioanr ao carrinho
function addToCart(name, price){
    const existingItem = cart.find(item => item.name === name)

    if(existingItem){
        // Se o item existir, apenas aumente 1 número com a quantidade
...
addressInput.addEventListener("input", function(event){
    ...
    if(inputValue !== ""){
        addressInput.classList.remove("border-zinc-500")
...
checkoutBtn.addEventListener("click", function(){
    ...
    if(addressInput.value === ""){
        ...
        addressInput.classList.add("border-zinc-500")
    }

    // enviar o peddio para api whats
